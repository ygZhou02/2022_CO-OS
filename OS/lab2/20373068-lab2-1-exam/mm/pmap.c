...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create) {
    ...
    if ((*pgdir_entry & PTE_V) == 0) {
        ...
        if (create) {
			...
			*pgdir_entry = PADDR(alloc(1ll<<14, 1ll<<14, 1));
            /**
			 * 
			*
    ...
    return ((Pte *)(/* II. Kernel Virtual Address of PTBase */KADDR(PTE_ADDR(*pgdir_entry)))) + PTX(va);
...
void boot_map_segment(Pde *pgdir,
    ...
    /* Hint: Use `boot_pgdir_walk` to ge*/
    Pte *pgtable_entry;
    for (i = 0, size = ROUND(size, BY2PG); i < size; i += BY2PG) {
        ...
        *pgtable_entry = (pa+i/* III. Physical Frame Address of `pa + i` */)     | perm | PTE_V;
    }
}
...
int page_alloc(struct Page **pp) {
  ...
  if (/* I. `page_free_list` is empty */ LIST_EMPTY(&page_free_list)) 
  		return -E_NO_MEM;

  // negative return value indicates exception.


  tmp = /* II. the first item in `page_free_list` */LIST_FIRST(&page_free_list);


  /* III. remove this page from the list */LIST_REMOVE(tmp, pp_link);


  bzero(/* IV. kernel virtual address of this page */page2kva(tmp), BY2PG);
...
void page_free(struct Page *pp) {
  ...
  if (pp->pp_ref == 0) {
    ...
    /* I. insert this item into [Math Processing Error] */
...
/*for lab2-1 exam*/
/*若 pp 对应的物理页面没有被保护，且处于空闲状态，则被永久保护 (即在其之后调用的
page_alloc 函数不会申请到这个物理页面)，并返回0。
若 pp 对应的物理页面没有被保护，且不处于空闲状态，返回-1。
若 pp 对应的物理页面已经被保护，返回-2。
初始状态下，所有物理页面都未被保护。*/
int page_protect(struct Page *pp)
{
	if(pp->pp_protect == 0 && pp->pp_ref == 0){
		pp->pp_protect = 1;
		LIST_REMOVE(pp, pp_link);
		return 0;
	}
	else if(pp->pp_protect == 0 && pp->pp_ref >0){
		return -1;
	}
	else return -2;
...
/*如果该物理页面被保护，状态为3。
如果该物理页面没有被保护且处于空闲状态，状态为2。
否则，状态为1。
*/
int page_status_query(struct Page *pp)
{
	if(pp->pp_protect) return 3;
	else if(pp->pp_protect ==0 && pp->pp_ref ==0)return 2;
	else return 1;
}
