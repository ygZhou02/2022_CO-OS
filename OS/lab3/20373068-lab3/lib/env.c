...
env_setup_vm(struct Env *e)
{
  ...
  for (i = PDX(UTOP); i < PTE2PT; i++)
    ...
    e->env_pgdir[PDX(UVPT)]  = e->env_cr3 | PTE_R | PTE_V;
    return 0;
}
...
env_alloc(struct Env **new, u_int parent_id)
{
    ...
    e->env_tf.regs[29] = USTACKTOP;   //设置为USTACKTOP
    e->env_runs=0;
							 ...
							 u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    //printf("binsize %x sgsize %x\n",bin_size, sgsize);
	struct Env *env = (struct Env *)user_data;
	struct Page *p = NULL;
	u_long i=0;
	int r;
	u_long offset = va - ROUNDDOWN(va, BY2PG);
    //printf("va is %x, offset is %x, va+offset is %x\n",va,offset,va+offset);
    ...
    if (offset)
	{
		p = page_lookup(env->env_pgdir, va-offset, NULL);
		if(p == 0)
		{
			r = page_alloc(&p);
			if (r != 0)
