...
int readelf(u_char *binary, int size)
{
        ...
        ptr_sh_table = binary + ehdr -> e_shoff; // note:
        sh_entry_count = ehdr -> e_shnum; // note:
        sh_entry_size = ehdr -> e_shentsize; // note:
        // for each section header, output section number and section addr.
        for (Nr = 0; Nr < sh_entry_count; Nr++) { // note:
                shdr = (Elf32_Shdr *)(ptr_sh_table + Nr * sh_entry_size); // note:
                printf("%d:0x%x\n", Nr, shdr->sh_addr); // note:
        } // note :
